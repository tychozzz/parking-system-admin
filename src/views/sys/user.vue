<template>
    <div>
        ...
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe @selection-change="handleSelectionChange">
            ...
            <el-table-column label="头像" width="50" key="0">
                <template slot-scope="scope">
                    <el-avatar size="small" :src="scope.row.avatar"></el-avatar>
                </template>
            </el-table-column>

            <el-table-column prop="username" label="用户名" width="120" key="1">
            </el-table-column>
            ...
            <el-table-column prop="email" label="邮箱" key="2">
            </el-table-column>

            <el-table-column prop="status" label="状态" key="3">
                <template slot-scope="scope">
            ...
            <el-table-column prop="created" width="200" label="创建时间" key="4">
            </el-table-column>
            <el-table-column prop="icon" width="260px" label="操作" key="5" v-if="!hideAllAction">
                <template slot-scope="scope">
                    <el-button type="text" v-if="hasAuth('sys:user:role')" @click="roleHandle(scope.row.id)">分配角色</el-button>
                    <!-- <el-divider direction="vertical"></el-divider> -->

                    <el-button type="text" v-if="hasAuth('sys:user:repass')" @click="repassHandle(scope.row.id, scope.row.username)">重置密码</el-button>
                    <!-- <el-divider direction="vertical"></el-divider> -->

                    <el-button type="text" v-if="hasAuth('sys:user:update')" @click="editHandle(scope.row.id)">编辑</el-button>
                    <!-- <el-divider direction="vertical"></el-divider> -->

                    <template>
                        <el-popconfirm title="这是一段内容确定删除吗？" @confirm="delHandle(scope.row.id)">
                            <el-button type="text" v-if="hasAuth('sys:user:delete')" slot="reference" style="margin-left: 10px;">删除
...
export default {
    ...
    methods: {
        isAuthorized() {
            console.log(this.hasAuth('sys:user:delete'))
            if (this.hasAuth('sys:user:role') 
                || this.hasAuth('sys:user:edit') 
                || this.hasAuth('sys:user:repass') 
                || this.hasAuth('sys:user:delete')) {
                this.hideAllAction = false
            } else {
        ...
        editHandle(id) {
            this.$axios.get('/sys/user/info/' + id).then(res => {
                this.editForm = res.data.data.sysUser
        ...
        roleHandle(id) {
            ...
            this.$axios.get('/sys/user/info/' + id).then(res => {
                this.roleForm = res.data.data.sysUser
                console.log(res)

                let roleIds = []
                res.data.data.sysUser.sysRoles.forEach(row => {
                    roleIds.push(row.id)
                })
